...
for(package in c('lme4', 'lmerTest', 'tidyverse', 'foreign', 'readr', 'sjstats')) { # start a loop
...
```{r echo = F}
knitr::kable(data_frame("Variable name" = c("grpid", "jobsat", "cohes", "posaff", "pay", "neglead", "wload", "tasksig", "physen"), 
...
p <- plot_ly(y = siop_merged$jobsat, x = siop_merged$posaff,
        z = siop_merged$physen) %>% 
  add_trace(type="scatter3d", showlegend = FALSE, opacity = 1,
            color = as.factor(siop_merged$physen))  %>% 
layout(scene = list(yaxis = list(title = 'Job satisfaction'),
                     xaxis = list(title = 'Positive affect'),
                     zaxis = list(title = 'Physical enviroment')))
