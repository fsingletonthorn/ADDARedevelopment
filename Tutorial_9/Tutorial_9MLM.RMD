...
First thing to know is if you have any problems at any point let one of the tutors know and we'll come to help out! 

The first thing you should do is unzip all of "tutorial_9.zip" into a new folder. You should then open up the "Tutorial_9.Rproj" file, without moving any of the contained files around. This is a way of opening R and R studio in a contained enviroment, so that all of the commands presented in this tutorial will work as intended. 
...
We can read in the data files using the function "read_csv". Copy paste the following code into your R console to read our data. 
...
You can pass multiple aguments to functions by separating them with commas, for example *round(1.123, 2)* rounds 1.123 to two decimal places, i.e., to 1.12.
<br>
<br>
</p>
</details>
...
*siop <- read_csv("siop.csv")* tells R to save the output of the function *read_csv("siop.csv")* as *siop* in the workspace (i.e., R's short term memory). 


We can **view** the files in Rstudio using the function *View()* (note the capital "V").

Notice we've got one file that is at the individual level (copy and paste the following code into your R session to see the data)
...
#### Merge siop and siop_group by the group variable, so that each individual row of data has a measure of the 

**NOTE:** you will need to change "Key_Column" to the column name that exists in both "siop" and "siop_group"

<details><summary>Hint</summary>  
<p>

*View(siop)* and *View(siop_group)* to see which columns names they have in common.
...
Copy and paste the following into R to merge the two files matching by *grpid*, the name of the group ID column in both "siop" and "siop_group".
...
#### **Make sure you've got siop_merged working before moving on, it should start like this:**
...
An overall intercept ($\gamma_0$), a random effect  for groups($u_{0j}$ i.e., error term), and an error term for each person in each group ($\epsilon_{ij}$). 
...
*Note that the error term is assumed*
...
This model says "how well can we model our data given that we're predicting the same score for every single person in a group, and saying that the group means can vary around some overall intercept". To visulize this see the plot below, which shows job satisfaction by group ID, with blue dots for the best linear unbiased predictor of each group's true mean. 
...
##### **Question 1**
How large is the intercept of the model? Looking at the dependent variable  (their mean, range and standard deviation) why might the value be so low? Should we worry about this?
...
##### **Question 2**
How big is the group level variance (the intercept variance)? Calculate the interclass correlation (you should do this by hand from the output using the slides from the lecture!)
...
a random effect for their group ($u_{0j}$, like an error term for group), plus an error term ($\epsilon_{ij}$) (i.e., the residual, as the MLM will not perfectly predict their score).

To write out this model in R all we need to do is add "+ posaff" (the variable name for positive affect) to the earlier null model. By tradition we'll put it after the intercept and before the random slopes.
...
ggplot(siop_merged, aes(y = jobsat, x = posaff, group = as.factor(grpid), colour = as.factor(grpid))) + 
  geom_point()  + xlab("Positive affect") + ylab("Job satisfaction") + 
  geom_abline(slope = PA_model@beta[2], intercept = coef(PA_model)[[1]][,1], colour = as.factor(1:50), alpha = .5) +
  theme_classic()+ theme(legend.position="none")
...
##### **Question 1**
How large is the intercept? Looking at the dependent variable  (their mean, range and standard deviation) why might the value be so low? Should we worry about this?
...
Looking at the model output, it looks like job satisfaction has been centered (i.e., altered to have a mean of ~ 0), but not obviously standardized (i.e., it doesn't have an SD of 1 for example). This is almost certainly fine.
...
## LME4 sytax / Design formula explainer 

The way that we input out models into LME4 and other model fitting functions in R is a notation form called a "desgin formula". This takes the expression for the predicted values, but doesn't include the parameter values, leaving instead just the predictor variable names seperated by "+" signs, or for multilvel models notation indicating how variables are nested together.

A good example is simple linear regression where we are predciting y with x. Normally we would think of this as something like $\hat{y_i} = a + \beta x_i$, i.e., the predicted value of y for person i is equal to some intercept a, plus $\beta$ times their score on $x$. 
In desing formual terms this becomes: 
