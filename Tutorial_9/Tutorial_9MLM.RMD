...
First thing to know is if you have any problems at any point let one of the tutors know and we'll come to help out! *Please also make sure that you have opened this document (lab9.html) in Google Chrome. If you do not have tabs up the top of this file, you should reopen this file in Google Chrome.*
...
This model say that each person's score is the product of their group membership plus random noise (i.e., the residuals). 

It assumes that the residuals are normally distrubited around a mean of zero, and we actually estimate the variance of the residuals, here `r round(as.data.frame(VarCorr(null_model))[2,4],3)`).
...
To write out this model in R all we need to do is add "+ posaff" (the variable name for positive affect) to the earlier null model. By tradition we'll put it after the intercept and before the random slopes. Run the following code to save the model output as PA_model.
